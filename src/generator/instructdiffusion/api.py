...
sys.path.append(os.path.join(BASE_DIR, "stable_diffusion"))
...
def instruct_diffusion_api(
        cfg_file    = "configs/instruct_diffusion.yaml",
        ckpt        = "checkpoints/v1-5-pruned-emaonly-adaption-task-humanalign.ckpt",
        resolution  = -1,
        input_img   = None,
        edit        = "change the color of the sky to red",
        seed        = 0,
        steps       = 50,
        cfg_text    = 7.0,
        cfg_image   = 1.25,
    ...
    if ckpt not in model_cache or (not use_cache):
        # load image
    ...
    model           = model_cache[ckpt]["model"]
    model_wrap      = model_cache[ckpt]["model_wrap"]
    model_wrap_cfg  = model_cache[ckpt]["model_wrap_cfg"]
    null_token      = model_cache[ckpt]["null_token"]
...
if __name__ == "__main__":
    ...
    output = instruct_diffusion_api(
        ...
        resolution  = 512,
        input_img   = "https://vidit98.github.io/publication/conference-paper/assets/sap_v3_e91/variation/2023-07-26T18-27-29/input.png",
        edit        = "make the door purple",
        seed        = 0,
        steps       = 50,
        cfg_text    = 3.5,
        cfg_image   = 1.25,
