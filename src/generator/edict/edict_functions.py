...
def EDICT_editing(im_path,
    ...
    orig_im = load_im_into_format_from_path(im_path, resolution=resolution) # if isinstance(im_path, str) else im_path # trust OK
    width, height = orig_im.size
    
    # compute latent pair (second one will be original latent if run_baseline=True)
    latents = coupled_stablediffusion(unet, vae, clip, clip_tokenizer,
                                      device=device,
                                      prompt=base_prompt,
                                      reverse=True,
                                      init_image=orig_im,
    ...
    gen = coupled_stablediffusion(unet, vae, clip, clip_tokenizer,
                                  device=device,
    ...
    gen = [g.resize((width, height), resample=Image.LANCZOS) for g in gen]
...
def load_im_into_format_from_path(im_path, resolution=-1):
    ...
    elif isinstance(im_path, Image.Image):
        im = im_path
    elif isinstance(im_path, np.ndarray):
        im = Image.fromarray(im_path)
    else:
        raise TypeError("im_path must be str or PIL.Image or np.ndarray")

    # crop using multiple of 64
    _width, _height = im.size

    if resolution > 0:
        width = resolution * _width // max(_width, _height)
        height = resolution * _height // max(_width, _height)
    ...
    # return center_crop(im).resize((512, 512))
...
def use_last_tokens_attention(use=True, unet=None):
    for name, module in unet.named_modules():
        module_name = type(module).__name__
        if module_name == "CrossAttention" and "attn2" in name:
...
def use_last_tokens_attention_weights(use=True, unet=None):
    for name, module in unet.named_modules():
        module_name = type(module).__name__
        if module_name == "CrossAttention" and "attn2" in name:
...
def use_last_self_attention(use=True, unet=None):
    for name, module in unet.named_modules():
        module_name = type(module).__name__
        if module_name == "CrossAttention" and "attn1" in name:
            module.use_last_attn_slice = use
...
def save_last_tokens_attention(save=True, unet=None):
    for name, module in unet.named_modules():
        module_name = type(module).__name__
        if module_name == "CrossAttention" and "attn2" in name:
...
def save_last_self_attention(save=True, unet=None):
    for name, module in unet.named_modules():
        module_name = type(module).__name__
        if module_name == "CrossAttention" and "attn1" in name:
            module.save_last_attn_slice = save
...
@torch.no_grad()
def baseline_stablediffusion(unet, vae, clip, clip_tokenizer, device="cuda",
                             prompt="",
                             prompt_edit=None,
                             null_prompt='',
    ...
    with autocast(device):
        ...
        if prompt_edit is not None:
            ...
            init_attention_edit(tokens_conditional, tokens_conditional_edit, clip_tokenizer=clip_tokenizer, device=device, unet=unet)
        elif prev_image is not None:
            init_attention_edit(tokens_conditional, tokens_conditional, clip_tokenizer=clip_tokenizer, device=device, unet=unet)
            
            
        init_attention_func(unet)
        init_attention_weights(prompt_edit_token_weights, clip_tokenizer=clip_tokenizer, device=device, unet=unet)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            if prompt_edit is not None or prev_latent is not None:
                ...
                use_last_tokens_attention_weights(unet=unet)
            ...
            if prev_latent is not None:
                ...
                noise_pred_cond = unet(
                    latent_model_input, t, encoder_hidden_states=embedding_conditional).sample

            #Edit the Cross-Attention layer activations
            t_scale = t / scheduler.num_train_timesteps
            if prompt_edit is not None or prev_latent is not None:
                if t_scale >= prompt_edit_tokens_start and t_scale <= prompt_edit_tokens_end:
                    use_last_tokens_attention(unet=unet)
                if t_scale >= prompt_edit_spatial_start and t_scale <= prompt_edit_spatial_end:
                    use_last_self_attention(unet=unet)
...
@torch.no_grad()
def coupled_stablediffusion(unet, vae, clip, clip_tokenizer, device="cuda",
                            prompt="",
                            prompt_edit=None,
                            null_prompt='',
                            prompt_edit_token_weights=[],
    ...
    def image_to_latent(im):
        if isinstance(im, torch.Tensor):
            ...
            if hasattr(Image, "Resampling"):
                im = im.resize((height, width), resample=Image.Resampling.LANCZOS)
            else:
    ...
    for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
        ...
        for latent_i in range(2): 
            ...
            #Prepare the Cross-Attention layers
            if prompt_edit is not None:
                save_last_tokens_attention(unet=unet)
                save_last_self_attention(unet=unet)
            else:
                #Use weights on non-edited prompt when edit is None
                use_last_tokens_attention_weights(unet=unet)
...
def main():
    ...
    results = coupled_stablediffusion(unet, vae, clip, clip_tokenizer, prompt='A black bear')
